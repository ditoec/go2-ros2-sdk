...
|---|---|
| Main driver | `colcon build --packages-select go2_robot_sdk` |
| Interfaces | `colcon build --packages-select go2_interfaces` |
| LiDAR (Python) | `colcon build --packages-select lidar_processor` |
| LiDAR (C++) | `colcon build --packages-select lidar_processor_cpp` |
| COCO detector | `colcon build --packages-select coco_detector` |
| Speech | `colcon build --packages-select speech_processor` |
...
| **Set Robot IP** | `export ROBOT_IP="192.168.x.x"` | Set target robot IP |
| **Set Conn Type** | `export CONN_TYPE="webrtc"` | WebRTC or cyclonedds |
...
| Topic | Command |
|---|---|
| Robot state | `ros2 topic echo /go2_state` |
| Joint states | `ros2 topic echo /joint_states` |
| IMU | `ros2 topic echo /imu` |
| Odometry | `ros2 topic echo /odom` |
| Point cloud | `ros2 topic hz /point_cloud2` |
| Laser scan | `ros2 topic hz /scan` |
| Send API cmd | `ros2 topic pub /webrtc_req go2_interfaces/msg/WebRtcReq "{api_id: 1016, topic: 'rt/api/sport/request'}" --once` |
...
| **Bag Record** | `ros2 bag record -a` | Record all topics |
| **Bag Play** | `ros2 bag play <bag_path>` | Replay a bag file |
